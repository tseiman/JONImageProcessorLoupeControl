namespace Loupedeck.JONImageProcessorLoupeControlPlugin
{
    ...
    public sealed class PauseFontAlignAdjustment : PluginDynamicAdjustment
    {
        private const Int32 DraftHoldSeconds = 10;
        private static readonly String[] DefaultOptions = ["left", "center", "right"];
        private IReadOnlyList<String> _options = DefaultOptions;
        private String _draftValue = "center";
        private DateTime _lastDraftChangeUtc = DateTime.MinValue;

        public PauseFontAlignAdjustment()
            : base(groupName: "Pause Text", displayName: "Font Align", description: "Selects pause.fontAlign; press the dial to apply", hasReset: true)
        {
            this.IsWidget = true;
        ...
        {
            this._draftValue = this.PauseControl.FontAlign;
            this.PauseControl.StateChanged += this.OnGatewayStateChanged;
            this.PauseControl.ConnectionChanged += this.OnGatewayConnectionChanged;
        ...
        protected override String GetAdjustmentDisplayName(String actionParameter, PluginImageSize imageSize) => "Align";

        protected override String GetAdjustmentValue(String actionParameter) => Title(this._draftValue);

        protected override BitmapImage GetAdjustmentImage(String actionParameter, PluginImageSize imageSize) =>
            DynamicFolderVisuals.CreateTextImage("Align", this.PauseControl.IsConnected, imageSize);
        ...
        private async System.Threading.Tasks.Task CommitAsync()
        {
            try
            {
                await this.PauseControl.SetFontAlignAsync(this._draftValue).ConfigureAwait(false);
                this._draftValue = this.PauseControl.FontAlign;
                this._lastDraftChangeUtc = DateTime.MinValue;
            }
            catch (Exception ex)
            {
                PluginLog.Warning($"[PauseFontAlignAdjustment] update failed: {ex.Message}");
            }
            finally
            {
                this.Refresh();
        ...
        private async System.Threading.Tasks.Task LoadOptionsAsync()
        {
            try
            {
                if (this.PauseControl.IsConnected)
                {
                    this._options = await this.PauseControl.GetFontAlignOptionsAsync().ConfigureAwait(false);
                    this.Refresh();
                }
            }
            catch (Exception ex)
            {
                PluginLog.Warning($"[PauseFontAlignAdjustment] schema load failed: {ex.Message}");
            }
        }

        private void OnGatewayStateChanged(Object sender, JonGatewayStateChangedEventArgs e)
        {
            if (!e.Values.ContainsKey(JonPauseControl.FontAlignKey))
            {
                return;
            }
