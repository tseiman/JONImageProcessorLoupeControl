namespace Loupedeck.JONImageProcessorLoupeControlPlugin
{
    ...
    public sealed class PauseFontSelectToggleCommand : PluginMultistateDynamicCommand, IBlinkenLightsReceiver, IMultiWheelDispatchable, IMultiWheelUploadCompletedHandler
    {
        ...
        private PauseFontScrollAdjustment _fontScrollAdjustment;
        private JonPauseControl _pauseControl;

        public PauseFontSelectToggleCommand()
            : base(groupName: "Wheel Select", displayName: "Pause Font ON/OFF", description: "Enables pause font selection with the big multi wheel")
        {
            this.IsWidget = true;
            JONImageProcessorLoupeControlPlugin.PluginReady += this.OnPluginReady;
            this.AddState("OFF", "Pause Font\nOFF", "Pause Font ON");
            this.AddState("ON", "Pause Font\nON", "Pause Font ON");
        }
        ...
        private void OnPluginReady()
        {
            ...
            this._fontScrollAdjustment = ServiceDirectory.Get(ServiceDirectory.T_PauseFontScrollAdjustment) as PauseFontScrollAdjustment;
            this._multiWheelDispatch?.RegisterDispatchable(this, this._fontScrollAdjustment);
        ...
        protected override void RunCommand(String actionParameter)
        {
            ...
            if (this.GetCurrentState().Name.Equals("ON", StringComparison.Ordinal))
            {
                ...
                _ = this._fontScrollAdjustment?.ReloadFontsAsync();
